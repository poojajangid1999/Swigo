<?php
/*
Template Name: YogaCare - Coach, Speaker & Motivation HTML Template
...
$dzEmailTo 		= "product@dexignzone.com";   /* Receiver Email Address */
$dzEmailFrom    = "YogaCare Contact";
...
function smtp_mail($dzEmailTo, $dzEmailFrom, $dzEmail, $dzMailSubject, $dzMailMessage)
{
    ...
    $mail->Host       = 'dexignzone.com';                //Set the SMTP server to send through
    $mail->SMTPAuth   = true;                                   //Enable SMTP authentication
    $mail->Username   = 'product@dexignzone.com';                   //SMTP username
    $mail->Password   = '88@=-+kAHwJ*';                        //SMTP password
...
try {
    if (!empty($_POST)) {
		...
		if($_POST['dzToDo'] == 'Contact')
		{
			...
			$dzMailSubject = 'YogaCare|Contact Form: A Person want to contact';
			$dzMailMessage	= 	"
		...
		if($_POST['dzToDo'] == 'Appointment')
		{
			...
			$dzMailSubject = 'YogaCare|Appointment Form: A Person want to contact';
			$dzMailMessage	= 	"
								A person want to contact you: <br><br>
								Name: $dzName<br/>
								Email: $dzEmail<br/>".$dzPhoneMessage."
